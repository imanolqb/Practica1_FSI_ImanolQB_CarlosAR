...
def graph_search(problem, fringe):
    ...
    visitedNodes = 0
    generatedNodes = 1
    fringe.append(Node(problem.initial))  # nodo raíz
    while fringe:
        node = fringe.pop()
        visitedNodes += 1
...
def branch_and_bound(problem):
    """Branch and Bound: Search nodes based on path cost, aiming for an optimal solution."""
    return graph_search(problem, branchAndBoundQueue())

def branch_and_bound_with_heuristic(problem):
    """Branch and Bound with Heuristic: Prioritize nodes based on path cost and heuristic estimates."""
    return graph_search(problem, branchAndBoundQueueHeuristic(problem))
...
romania = UndirectedGraph(Dict(
    A=Dict(Z=75, S=140, T=118),                     # Arad
    B=Dict(U=85, P=101, G=90, F=211),               # Bucharest
    C=Dict(D=120, R=146, P=138),                    # Craiova
    D=Dict(M=75),                                   # Dobreta
    E=Dict(H=86),                                   # Eforie
    F=Dict(S=99),                                   # Fagaras
    H=Dict(U=98),                                   # Hirsova
    I=Dict(V=92, N=87),                             # Iasi
    L=Dict(T=111, M=70),                            # Lugoj
    O=Dict(Z=71, S=151),                            # Oradea
    P=Dict(R=97),                                   # Pitesti
    R=Dict(S=80),                                   # Rimnicu Vilcea
    U=Dict(V=142)))                                 # Urziceni
